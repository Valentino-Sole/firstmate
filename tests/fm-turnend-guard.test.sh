...
test_tracked_claude_entries_inert_under_grok() {
  ...
  for script in fm-turnend-guard.sh fm-claude-stop-autoarm.sh fm-sessionstart-run.sh \
    fm-arm-pretool-check.sh fm-cd-pretool-check.sh fm-primary-checkout-pretool-check.sh fm-klartext-uebernahme-pretool-check.sh fm-subagent-pretool-check.sh; do
  ...
  [ "$guarded" -eq 7 ] || fail "expected 7 grok-guarded tracked entries (5 upstream plus the primary-checkout and Klartext-Uebernahme seatbelts), saw $guarded"
...
test_pi_extension_injects_once_per_logical_agent_run() {
  ...
  cp "$ROOT/.pi/extensions/lib/fm-agents-refresh.ts" "$repo/.pi/extensions/lib/fm-agents-refresh.ts"
