...
state/               runtime records and signals; gitignored
  ...
  inbox/             captain notes captured out of band by bin/fm-inbox.sh, including the voice handover's queued requests; each note appends one `check` wake and stays pending until acknowledged with `bin/fm-inbox.sh drain --ack <id>`, which moves it to inbox/handled/ (docs/voice-relay.md); `bin/fm-inbox.sh reply <id> <text>` answers one note durably and keyed by its own id under inbox/replies/, and itself acknowledges the note the same way; `note --key <token>`/`reply --key <token>` let an external bridge (JARVIS or similar) carry its own conversation key as the note's own `external_key=` header field instead of folding it into the free text
