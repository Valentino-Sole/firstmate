...
config/worker-slots-max  optional hard ceiling on independent workers running on this host at once, counted across every local home; LOCAL, gitignored, read from the PRIMARY home (a secondmate home obeys its parent's file); see docs/configuration.md "Worker slot cap"
...
state/               runtime records and signals; gitignored
  ...
  inbox/             captain notes captured out of band by bin/fm-inbox.sh, including the voice handover's queued requests; each note appends one `check` wake and stays pending until acknowledged with `bin/fm-inbox.sh drain --ack <id>`, which moves it to inbox/handled/ (docs/voice-relay.md); `bin/fm-inbox.sh reply <id> <text>` answers one note durably and keyed by its own id under inbox/replies/, and itself acknowledges the note the same way; `note --key <token>`/`reply --key <token>` let an external bridge (JARVIS or similar) carry its own conversation key as the note's own `external_key=` header field, or as JARVIS's own legacy footer key, instead of firstmate's internal id; for a keyed note, `reply` also attempts real HTTP delivery to that bridge and marks confirmed deliveries under inbox/replies/delivered/, retried on failure with `bin/fm-inbox.sh deliver <id>` (`bin/fm-inbox.sh --help` owns the full contract)
