...
Neither provider's behavior originates in Firstmate's own tracked `.pi/extensions/` or `bin/fm-session-start.sh`; both were independently verified to deliver their session-start digest at most once per session generation, capped at 512 KiB.
