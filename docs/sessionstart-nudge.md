...
| Cursor compaction | Uncovered (digest) | `preCompact` marks `state/.cursor-compaction` so the stop park will not submit a follow-up during compact. | Digest re-emit remains deferred: `preCompact` can return only `user_message` and cannot inject `additional_context`. A compacted Cursor session keeps whatever context survived rather than receiving a fresh digest. |
...
Cursor's compaction digest surface stays uncovered in the same sense as Codex's interactive TUI above: `preCompact` cannot inject a re-emit digest, so a compacted Cursor session keeps whatever context survived rather than receiving a fresh digest.
The same hook is registered only to mark compaction active for the stop-park follow-up hold.
