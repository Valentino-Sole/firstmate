...
Cursor rejects a `followup_message` submit while it is compacting, so tracked `preCompact` marks `state/.cursor-compaction` and the park holds exactly one follow-up in `state/.cursor-compaction-held` until `afterAgentResponse` clears the mark, then submits that object once.
...
The once-only ceiling notice takes the slot from a replaceable held event when the window outlasts the wait, rather than being dropped: a wake stays durable in the queue and a nag is bounded, but that notice is the session's single warning that automatic delivery stops.
