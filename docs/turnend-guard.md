...
That age is measured against the window as well as the clock: `afterAgentResponse` stamps `state/.cursor-compaction` closed instead of deleting it, and a held event of the same session stays deliverable while that stamp is younger than the budget.
