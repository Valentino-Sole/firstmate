...
Their separate owners are [`arm-pretool-check.md`](arm-pretool-check.md), [`cd-guard.md`](cd-guard.md), [`primary-checkout-guard.md`](primary-checkout-guard.md), and [`subagent-guard.md`](subagent-guard.md).
...
That hook remains unregistered.
Cursor rejects a `followup_message` submit while it is compacting, so tracked `preCompact` marks `state/.cursor-compaction` and the park holds exactly one follow-up in `state/.cursor-compaction-held` until `afterAgentResponse` stamps that mark closed, then submits that object once.
...
Writing the hold is a state mutation like the output itself, so it happens inside the same guarded section as every other one, under the same still-held owner lock as the park ownership, session ownership and away-mode checks; away mode and a newer stop's claim take that same lock, so neither can interleave between the check and the hold, and a superseded park, a foreign session or away mode leaves nothing behind for a later stop to deliver.
The held record carries the budget action its eventual submit must apply, because the stop that delivers it is usually not the stop that built it: an actionable wake still resets the repair-nag budget when it is delivered out of the hold, and a failed reset re-parks the event instead of discarding it.
It also carries the session that parked it and its own `updated_at`, and only that session may submit it while it is younger than the same budget the mark expires by, so a follow-up built for one session is never replayed into the next one; an expired or foreign record blocks nothing, because the next hold replaces it.
That age is measured against the window as well as the clock: `afterAgentResponse` stamps `state/.cursor-compaction` closed instead of deleting it, and a held event stays deliverable while the stamp closing *its own* window is younger than the budget.
Without it a window that outlasts the park's wait would expire the event it parked, because that park submits nothing, no further turn starts, and the stop that could finally deliver it only arrives when the captain types.
The grace is keyed to the window the event was parked inside, so an unrelated window opening and closing hours later cannot revive an event that expired long before it, and stranding still cannot outlive one window.
The once-only ceiling notice gets one more chance at the slot when the window outlasts the wait, so a free slot parks it instead of dropping it; it never evicts an event that is still deliverable, because that one is itself owed a submit and the single slot cannot owe two.
That is an accepted gap rather than a guarantee: when the slot already holds a deliverable event and the window outlasts the wait, the ceiling notice is absorbed instead of printed, and because `loop_count` passes its ceiling exactly once it is not rebuilt later.
Reaching it needs a `preCompact` that refreshes the mark during the wait or `FM_CURSOR_COMPACTION_MAX_AGE` configured above `FM_CURSOR_COMPACTION_WAIT_MAX`; with the defaults equal, the mark expires as the wait ends and the notice prints.
The terminal notice deliberately gets no delivery path of its own outside the shared slot.
...
A repair nag charges `FM_CURSOR_TURNEND_BLOCK_BUDGET` in that same section at the moment the hold slot or the print accepts it, so a nag carried across a compaction window numbers itself correctly and the bound still stops at three, while a nag the occupied slot refused and no print delivered stays unspent.
...
- Cursor's `preCompact` step cannot inject a digest (`user_message` only, absent from `additional_context`), so post-compaction re-emit stays deferred ([`sessionstart-nudge.md`](sessionstart-nudge.md)).
  The same step is registered only to mark compaction active so the park will not submit a follow-up during that window.
