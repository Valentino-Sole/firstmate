...
Cursor's `beforeSubmitPrompt` step fires once for a real captain message and not for hook-driven follow-ups, so it remains unregistered.
While Cursor is compacting, the park does not submit a new follow-up.
`preCompact` marks that window, the one event is held, and after a successful compact it is submitted exactly once.
