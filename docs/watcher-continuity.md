...
Recovery therefore keeps the text it just resubmitted, and a captain submission of exactly that text is transformed through the `input` event into the same words behind a harness notice that states the duplication and asks for a single execution.
The captain's own words are kept verbatim behind that notice rather than dropped, because swallowing their submission would be the lost input this whole mechanism exists to prevent, and the notice is plain text rather than an operational envelope, so the marked resend still establishes an ordinary captain boundary.
It is also recorded for tracking as it will be appended, not as it was typed, so the transcript comparison and the `before_agent_start` commit still match it exactly and a marked resend is never judged lost itself.
