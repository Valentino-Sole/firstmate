...
The handler therefore counts logical runs in flight - incremented on `before_agent_start`, which Pi emits only for a genuinely new run and never for a message queued into an active one - and returns without evaluating anything while that count is still above zero, so both the supervision check and reply recovery are judged only on the settle that drains it.
