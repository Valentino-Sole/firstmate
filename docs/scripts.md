...
| `fm-sessionstart-run.sh` | Route a native session-open hook to the full digest, a context re-emit, a compact continuation note, or the nudge |
...
| `fm-work-loop.sh`        | Report measured free worker slots, real-worker floor, and plan parallel backlog refill |
