...
The checkout under test is the one holding the running script, not whatever `FM_ROOT_OVERRIDE` names: an override that resolves to a different checkout is a parent primary's environment inherited by a crew, scout, or secondmate session, so `fm_primary_scope_env_is_foreign` (`bin/fm-primary-scope-lib.sh`, shared with the turn-end guard) rejects it and the guard scopes itself to its own checkout instead.
Without that, a crewmate would be denied a `cd` inside its own worktree.
...
The suite also proves the end-to-end cwd-leak regression (a firstmate-owned backlog write leaking into a project clone, then denied at the exact command), the checkout scoping (fires in a git-cloned secondmate fixture, inert in a crewmate/scout linked worktree, inert in that worktree even when it inherited the parent primary's `FM_ROOT_OVERRIDE`, still fires when the override names its own checkout, inert for an unresolvable override, inert outside a firstmate checkout, inert outside a git repo), the fail-open transport behavior, the prefilter fast path, the policy CLI output contract, and the per-harness wiring.
