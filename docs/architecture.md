...
The pool decides a slot is free from the processes it can see inside it, not from firstmate's records, so `fm-spawn.sh` additionally refuses any pooled copy that another task's still-present `state/<id>.meta` records as its worktree and asks the pool for a different one.
Portable regressions live in [`tests/fm-spawn-pool-base-freshen.test.sh`](../tests/fm-spawn-pool-base-freshen.test.sh) for spawn isolation and base freshness, [`tests/fm-spawn-pool-collision.test.sh`](../tests/fm-spawn-pool-collision.test.sh) for the occupied-copy refusal, and [`tests/fm-control-relaunch.test.sh`](../tests/fm-control-relaunch.test.sh) for preserving the recorded copy on relaunch.
...
Foreign-source archival copies captured under domain-expiry pressure (collectosk.com, then tcdb.com) follow the same isolation logic: captain decision 2026-09-03 keeps them as reference/backup copies only, never automatically adopted as checklist truth and never sealed into a closed decision; `docs/fremdquellen-quellenrisiko.md` owns that contract.
