...
Any genuine state change ends that episode and opens a new one that reports immediately again: the pane going busy, its capture hash changing, a declared pause or captain-held transfer taking over, and the crew writing its own task worktree all drop the episode's resurface throttle, so a pane that goes from deferred-writing back to genuinely wedged surfaces on the next escalation rather than waiting out the previous episode's `FM_PAUSE_RESURFACE_SECS` clock.
