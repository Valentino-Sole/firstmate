...
`fm-spawn.sh` unsets all five `FM_*_OVERRIDE` variables in the literal launch line of every spawned pane, regardless of task kind, so a pane can never inherit a foreign home's overrides from elsewhere in the launching process tree; the reset is a real `unset` rather than an empty assignment, because a consumer that distinguishes unset from empty (such as `bin/fm-check-unregister.sh`, which refuses an explicitly empty override) must still see the variable as unset. Because a pane runs whichever login shell the operator uses, the launch line carries that reset twice: the POSIX `unset` plus, behind a `status fish-path` guard that only a fish pane can satisfy, fish's `set --erase` spelling - on a fish pane `unset` is not a builtin at all and would otherwise leave the reset a silent no-op. A secondmate launch additionally redirects `FM_HOME` to the secondmate's own home alongside that reset.
