...
`fm-spawn.sh` explicitly clears all five `FM_*_OVERRIDE` variables in the literal launch line of every spawned pane, regardless of task kind, so a pane can never inherit a foreign home's overrides from elsewhere in the launching process tree; a secondmate launch additionally redirects `FM_HOME` to the secondmate's own home alongside that reset.
