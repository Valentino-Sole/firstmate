...
Those host gates are also what rejects an unmeasurable host: the POSIX probe reports `mem_avail_mb=0` alongside `load1=0` when it cannot read `/proc`, so that fabricated zero load fails on the RAM half, and a host that returned no usable CPU sample has no headroom measurement to pass at all.
