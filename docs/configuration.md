...
`fm-spawn.sh` unsets all five `FM_*_OVERRIDE` variables in the literal launch line of every spawned pane, regardless of task kind, so a pane can never inherit a foreign home's overrides from elsewhere in the launching process tree; the reset is a real `unset` rather than an empty assignment, because a consumer that distinguishes unset from empty (such as `bin/fm-check-unregister.sh`, which refuses an explicitly empty override) must still see the variable as unset. A secondmate launch additionally redirects `FM_HOME` to the secondmate's own home alongside that reset.
