...
The branch prompt embeds its own complete verdict list, so the branch never re-derives that list from [`AGENTS.md`](../AGENTS.md) or another rules file at wake time; the list matches the section 7 result-loop captain gates plus the operational escalations supervision owns, and summaries use captain outcome language without internal mechanics.
